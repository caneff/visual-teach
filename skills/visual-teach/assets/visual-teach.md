...
| bpmn        | the lesson explains who-hands-what-to-whom across actors (request lifecycles, auth flows, producer/queue/consumer, CI pipelines, branch-with-consequences) — not for single-actor pipelines (diagram's vt-flow), state machines (mermaid state), or >5 lanes / >12 nodes | [components/bpmn/demo.html](components/bpmn/demo.html)               |
