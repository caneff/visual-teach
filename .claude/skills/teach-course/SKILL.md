...
This skill exercises the `teach-base` skill non-interactively. `teach-base` normally interviews the user (mission, zone of proximal development, preferences). Here you answer all of those yourself with sensible defaults and produce a finished 3-lesson course in one pass. **Do not ask the user anything except the subject.**

## Procedure

1. **Invoke `teach-base`** via the Skill tool. Everything below operates inside that skill's workspace conventions and mode rules (Ponytail OFF, humanize all reader-facing prose). Treat the current directory as the teaching workspace.

1b. **Run the visual-teach setup before invoking teach-base.** Mandatory on every course initialization. Seed the workspace `./assets/` with the visual-teach component library so teach-base discovers and reuses it under its own component rule:
...
4. **Author exactly 3 lessons**, `lessons/0001-*.html` through `0003-*.html`, following all of teach-base's lesson rules (beautiful Tufte-style HTML, shared stylesheet in `./assets/`, citations, primary source, recall/practice feedback loop, followup reminder, anchor links between lessons). Sequence them as a coherent arc in the auto-generated mission's zone of proximal development:
...
A course test is a smoke test of the `teach-base` skill, not a gap-hunting audit of
...
## Defaults (use when teach-base would ask)
...
Report the created files and one line on the assumed mission, so the user can correct it and continue in a normal `teach-base` session.
